...
use proof::conservation_proof::{ProofBundle, ProofChain};
...
fn verify_proof_chain(file_path: &str) -> PyResult<bool> {
    let json_data = fs::read_to_string(file_path).map_err(|e| {
        pyo3::exceptions::PyIOError::new_err(format!("Failed to read proof file: {}", e))
    })?;

    let bundle: ProofBundle = serde_json::from_str(&json_data).map_err(|e| {
